...
import type { IResource, IResourceResponse, ItemsType } from "~/types"
...
const { data, pending, status, error, refresh } = useLazyAsyncData(
  "challenges",
  async () => {
    const data = await apiFetch<IResourceResponse>(
      `/bookmarks/${type.value}/`,
      {
        params: {
          page: page.value
        }
  ...
  {
    transform: (data): IResourceResponse => {
      return {
        ...data,
        results: data.results.map((item) => {
          return {
            ...item,
            type: adjustType(type.value)
          } as IResource
        })
      }
    }
...
<template>
  <main class="py-8">
    <div class="container">
      <MainHeading>Cyber Defenders {{ type }}</MainHeading>
